...
Registers four ZenMux provider IDs that share a single `ZENMUX_API_KEY` and route over the correct native transport for the upstream model. Anthropic models get native prompt caching automatically; OpenAI and Vertex models use OpenAI-compatible chat completions.
...
| `zenmux-vertex`    | `openai-completions`  | `https://zenmux.ai/api/vertex-ai/v1beta`  |
